import { MembershipLevel, LadyMembershipLevel, AnyMembershipLevel } from '../types';
...
export const getMaxReviewCount = (
  level: AnyMembershipLevel | undefined,
  isVip: boolean,
  ...
  // 只處理茶客等級（MembershipLevel），佳麗等級（LadyMembershipLevel）不適用
  ...
  // 如果不是茶客等級，返回0
  if (!clientLevels.includes(level as MembershipLevel)) {
    return 0;
  }

  const membershipLevel = level as MembershipLevel;
  ...
  if (isPremiumTea) {
    ...
    const levelIndex = premiumTeaLevels.indexOf(membershipLevel);
    if (levelIndex === -1) {
      return 0; // 不在列表中，返回0
    }
  ...
  const levelIndex = fishMarketLevels.indexOf(membershipLevel);
  if (levelIndex === -1) {
    return 0; // 不在列表中，返回0
  }
 ...
 * @returns 等級索引（1開始）
 */
export const getLevelIndex = (level: MembershipLevel | undefined): number => {
  if (!level) {
    return 0;
  }
