      ...
      <div className="p-4 md:p-8">
            ...
            <div className="mb-8 border-b border-gray-100 pb-6">
              ...
              {stats && stats.reviewsCount > 0 && (
                <div className="flex flex-wrap items-center gap-3 mb-3 text-sm">
                  <span className="inline-flex items-center gap-1 px-3 py-1 bg-yellow-50 text-yellow-700 rounded-full font-bold">
                    <svg className="w-4 h-4 text-yellow-400 fill-current" viewBox="0 0 24 24"><path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>
              ...
              <div className="mb-4 flex items-center gap-3 p-3 bg-gradient-to-r from-pink-50 to-red-50 rounded-2xl border-2 border-pink-200">
                ...
                <div className="flex-1">
                  <p className="text-sm font-bold text-gray-800">點 <svg className="w-4 h-4 text-pink-500 fill-current inline-block" viewBox="0 0 24 24"><path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg> 加入我的最愛</p>
        ...
        <div className="mt-16 border-t border-gray-100 pt-10">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-2xl font-serif font-black text-brand-black flex items-center gap-2">
              <svg className="w-6 h-6 text-yellow-400 fill-current" viewBox="0 0 24 24"><path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>
              評論 {totalReviews > 0 && <span className="text-base text-gray-400 font-normal">({totalReviews})</span>}
            </h3>
          ...
          {!isAuthenticated ? (
            <div className="text-center py-8 bg-gray-50 rounded-2xl border border-gray-100">
              <p className="text-gray-500 text-sm">登入後即可查看和撰寫評論</p>
            </div>
          ) : isLoadingReviews ? (
            <div className="text-center py-8 text-gray-400 text-sm">載入評論中...</div>
            ...
            <div className="space-y-4">
              {visibleReviews.map(review => (
                <ReviewCard
                  key={review.id}
                  review={review}
                  canInteract={isAuthenticated && user?.role === 'client'}
                  onLikeChange={() => loadReviews()}
                />
              ))}
              {totalReviews > visibleReviews.length && !showAllReviews && (
                <button
                  onClick={() => setShowAllReviews(true)}
                  className="w-full py-3 text-sm text-gray-500 hover:text-gray-700 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors"
                >
                  查看全部 {totalReviews} 則評論
                </button>
              )}
            </div>
          ) : (
            <div className="text-center py-8 bg-gray-50 rounded-2xl border border-gray-100">
              <p className="text-gray-400 text-sm">尚無評論，成為第一個評論的人吧</p>
            </div>
          )}
        </div>
